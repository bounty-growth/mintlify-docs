...
For MCP clients, see [MCP Server](/cli/mcp). The MCP server uses a separate `mcp:read` bearer token that wraps the same kind of authenticated Bounty user session.
